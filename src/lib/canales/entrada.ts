...
export const MENSAJE_NO_ENCONTRADO =
  "No encontré ese canal. Revisa el enlace o abre el canal en YouTube y copia la dirección desde la barra del navegador.";
...
export function analizarEntrada(crudo: string): EntradaAnalizada {
  ...
  if (!t) return { error: "Escribe o pega el enlace de un canal de YouTube." };
  ...
  if (!conEsquema) {
    return { error: "Eso no parece un enlace de YouTube. Pega la dirección de un canal, de una lista o de un video." };
  }
  ...
  try {
    ...
    return { error: "Ese enlace no está bien escrito. Cópialo de nuevo desde la barra de direcciones de YouTube." };
  }
  if (!esHostYouTube(url.hostname)) {
    return { error: "Ese enlace no es de YouTube. Pega la dirección de un canal, de una lista o de un video." };
  }
  ...
  if (/^youtu\.be$/i.test(url.hostname)) {
    return ID_VIDEO.test(primero) ? { entrada: { tipo: "video", id: primero } } : { error: MENSAJE_NO_ENCONTRADO };
  }

  if (primero === "channel") {
    return ID_CANAL.test(seg[1] ?? "") ? { entrada: { tipo: "canal-id", id: seg[1]! } } : { error: MENSAJE_NO_ENCONTRADO };
  }

  const lista = url.searchParams.get("list");
  if (primero === "playlist") {
    return lista && ID_LISTA.test(lista) ? { entrada: { tipo: "lista", id: lista } } : { error: MENSAJE_NO_ENCONTRADO };
  }
  ...
  if (primero === "watch") {
    ...
    return { error: MENSAJE_NO_ENCONTRADO };
  }

  if (["shorts", "embed", "live", "v"].includes(primero)) {
    return ID_VIDEO.test(seg[1] ?? "") ? { entrada: { tipo: "video", id: seg[1]! } } : { error: MENSAJE_NO_ENCONTRADO };
  }
  ...
  return { error: "Ese enlace de YouTube no apunta a un canal, una lista ni un video. Abre el canal y copia su dirección." };
}
