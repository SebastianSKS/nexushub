import { localeActual, traducir } from "@/lib/i18n";
const dias = () => new Intl.RelativeTimeFormat(localeActual(), { numeric: "auto" }); // "ayer", "hace 3 días" / "yesterday", "3 days ago"
const resto = () => new Intl.RelativeTimeFormat(localeActual(), { numeric: "always" }); // "hace 1 año" / "1 year ago"

/**
 * Fecha relativa en el idioma de ahora: "hace 3 días", "hace 2 semanas".
...
export function fechaRelativa(iso: string, ahora: number = Date.now()): string {
  ...
  if (s < 60) return traducir("hace un momento");
