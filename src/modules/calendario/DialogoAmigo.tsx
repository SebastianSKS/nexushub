...
export function DialogoAmigo({ abierto, inicial, onCerrar }: { abierto: boolean; inicial: BorradorAmigo | null; onCerrar: () => void }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => campoNombre.current?.focus(), 120);
  ...
  const guardar = (e: FormEvent) => {
    ...
    if (!nombre.trim()) nuevos.nombre = "Escribe el nombre de tu amigo.";
    const anioNum = anio.trim() ? Number(anio) : null;
    if (anioNum !== null && (!Number.isInteger(anioNum) || anioNum < 1900 || anioNum > new Date().getFullYear())) nuevos.anio = `Escribe un año entre 1900 y ${new Date().getFullYear()}, o déjalo vacío.`;
    setErrores(nuevos);
  ...
  return (
    <Dialog open={abierto} onClose={onCerrar} title={editando ? "Editar cumpleaños" : "Añadir cumpleaños"} maxWidth={480}>
      <form onSubmit={guardar} className="flex flex-col gap-4">
        <TextInput ref={campoNombre} label="Nombre del amigo" value={nombre} maxLength={40} autoComplete="off" placeholder="Por ejemplo, Ana" error={errores.nombre} onChange={(e) => { setNombre(e.target.value); setErrores((x) => ({ ...x, nombre: undefined })); }} />
        ...
        {mes === 2 && dia === 29 && <p className="-mt-2 text-caption text-fg-tertiary">En los años que no son bisiestos se marcará el 28 de febrero.</p>}
        ...
        <div>
          ...
          <input id={idNota} value={nota} maxLength={200} placeholder="Idea de regalo, cómo lo conociste…" onChange={(e) => setNota(e.target.value)} className={CAMPO} />
        </div>

        <div className="flex items-center justify-between gap-4">
          <span className="text-body text-fg">Avisarme de este cumpleaños</span>
          <Switch checked={avisar} onChange={setAvisar} label="Avisarme de este cumpleaños" />
        </div>
          ...
          {editando ? (
            confirmarBorrado ? (
              <span className="flex items-center gap-2">
                <span className="text-caption text-fg-secondary">¿Eliminar a {inicial?.nombre}?</span>
