  ...
  const importar = async (files: File[]) => {
    ...
    try {
      ...
      if (nuevosAmigos.length === 0 && nuevosEventos.length === 0) {
        setError(t("No encontré nada que importar en ese archivo."));
        return;
      }
      ...
      setError(t("No pude leer ese archivo. Comprueba que sea un .ics válido."));
    }
  };
  ...
  return (
    <Card className="p-4">
      <h2 className="mb-1 flex items-center gap-2 text-body font-semibold text-fg">
        <Glifo nombre="carpeta" tam={16} className="text-accent-text" /> {t("Copia de seguridad")}
      </h2>
      <p className="mb-3 text-caption text-fg-secondary">{t("Exporta tu calendario o impórtalo desde Google Calendar, Outlook u otra aplicación (.ics).")}</p>
      ...
      <Dialog open={resultado !== null} onClose={() => setResultado(null)} title={t("Importación terminada")} maxWidth={420}>
        {resultado && (
          <>
            <p className="text-body text-fg-secondary">
              {t("Se añadieron {a} cumpleaños y {e} {eventos}.", { a: resultado.amigos, e: resultado.eventos, eventos: resultado.eventos === 1 ? t("evento") : t("eventos") })}
              {resultado.omitidos > 0 && ` ${resultado.omitidos === 1 ? t("Se dejó fuera 1 entrada repetida o incompleta.") : t("Se dejaron fuera {n} entradas repetidas o incompletas.", { n: resultado.omitidos })}`}
            </p>
            <div className="mt-5 flex justify-end">
              <Button variant="accent" onClick={() => setResultado(null)}>{t("Entendido")}</Button>
            </div>
          </>
        )}
      </Dialog>
