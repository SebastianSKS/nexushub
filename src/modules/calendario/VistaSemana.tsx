...
export function VistaSemana({ fechaBase, amigos, eventos, onSemana, onHoy, onDia, onAmigo, onEvento }: Props) {
  ...
  const titulo = mismoMes
    ? `${dias[0]!.getDate()} – ${dias[6]!.getDate()} de ${MESES[dias[0]!.getMonth()]}`
    : `${dias[0]!.getDate()} de ${MESES[dias[0]!.getMonth()]} – ${dias[6]!.getDate()} de ${MESES[dias[6]!.getMonth()]}`;

  return (
    <section aria-label={`Semana del ${titulo}`} className="overflow-hidden rounded-[8px] border border-stroke bg-layer shadow-card">
      <header className="flex items-center gap-2 px-4 py-3" style={{ backgroundImage: "linear-gradient(135deg, #0f6cbd 0%, #2b88d8 60%, #4aa8ee 100%)" }}>
        ...
        <button type="button" onClick={() => onSemana(-1)} aria-label="Semana anterior" title="Semana anterior" className="rounded-control flex h-8 w-8 items-center justify-center text-white transition-colors duration-exit ease-fluent hover:bg-white/15">
          <Glifo nombre="chevronIzquierda" tam={14} />
        </button>
        <button type="button" onClick={() => onSemana(1)} aria-label="Semana siguiente" title="Semana siguiente" className="rounded-control flex h-8 w-8 items-center justify-center text-white transition-colors duration-exit ease-fluent hover:bg-white/15">
          ...
          return (
            <li key={fecha.toISOString()} className={clsx("flex gap-4 p-4", esHoy && "bg-layer-alt")}>
              <div className="w-[92px] shrink-0">
                <span className={clsx("block text-body font-semibold", esHoy ? "text-accent-text" : "text-fg")}>{mayuscula(fechaLarga(fecha).split(" ")[0]!)}</span>
