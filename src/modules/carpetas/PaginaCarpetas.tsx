...
function DialogoNombre({ titulo, etiqueta, inicial, accion, onGuardar, onCerrar }: { titulo: string; etiqueta: string; inicial: string; accion: string; onGuardar: (nombre: string) => Promise<void>; onCerrar: () => void }) {
  ...
  const enviar = async (e: FormEvent) => {
    e.preventDefault();
    if (!texto.trim()) return setError(t("Escribe un nombre."));
    setOcupado(true);
    try {
      await onGuardar(texto.trim());
      onCerrar();
    } catch (err) {
      setError(err instanceof ErrorCarpetas ? err.message : t("No se pudo guardar."));
    } finally {
      setOcupado(false);
    }
  };
...
const fecha = (seg: number) => new Date(seg * 1000).toLocaleDateString(localeActual(), { day: "numeric", month: "short", year: "numeric" });
...
export function PaginaCarpetas() {
  ...
  const recargar = useCallback(async () => {
    try {
      ...
      setAviso({ tipo: "error", texto: e instanceof ErrorCarpetas ? e.message : t("No se pudieron leer las carpetas.") });
      setCarpetas([]);
    }
  }, []);
  ...
  const cargarArchivos = useCallback(async (carpeta: string) => {
    try {
      ...
      setAviso({ tipo: "error", texto: e instanceof ErrorCarpetas ? e.message : t("No se pudo leer la carpeta.") });
      setAbierta(null);
    }
  }, []);
  ...
  const intentar = async (accion: () => Promise<void>, exito?: string) => {
    try {
      ...
      setAviso({ tipo: "error", texto: e instanceof ErrorCarpetas ? e.message : t("No se pudo completar.") });
    }
  };
  ...
  const agregar = async (lista: FileList | File[]) => {
    ...
    await intentar(async () => {
      ...
      await recargar();
    }, (archivosNuevos.length === 1 ? t("Archivo guardado en «{carpeta}».", { carpeta: abierta }) : t("{n} archivos guardados en «{carpeta}».", { n: archivosNuevos.length, carpeta: abierta })));
  };
  ...
  return (
    <>
      <PlantillaPagina
        migas={abierta ? [{ etiqueta: t("Documentos"), href: "/documentos" }, { etiqueta: t("Mis tareas"), href: "/documentos/carpetas" }, { etiqueta: abierta }] : [{ etiqueta: t("Documentos"), href: "/documentos" }, { etiqueta: t("Mis tareas") }]}
        titulo={abierta ?? "Mis tareas"}
        descripcion={abierta ? t("Los archivos de esta materia. Están en tu computadora, en una carpeta de verdad.") : t("Una carpeta por materia para guardar tus tareas y trabajos. Están en tu computadora, en Documentos › Nexo › Tareas.")}
        accion={
          disponible && (
            <div className="flex flex-wrap justify-end gap-2">
              {abierta ? (
                <>
                  <Button onClick={() => void intentar(() => abrirEnSistema(abierta))} icon={<Glifo nombre="carpeta" />}>{t("Abrir en el Explorador")}</Button>
                  <Button icon={<Glifo nombre="agregar" />} onClick={() => setDialogo({ tipo: "nuevo-archivo" })}>{t("Nuevo archivo")}</Button>
                  <Button variant="accent" icon={<Glifo nombre="agregar" />} onClick={() => entrada.current?.click()}>{t("Añadir archivos")}</Button>
                </>
              ) : (
                <>
                  <Button onClick={() => void intentar(() => abrirEnSistema())} icon={<Glifo nombre="carpeta" />}>{t("Abrir en el Explorador")}</Button>
                  <Button variant="accent" icon={<Glifo nombre="agregar" />} onClick={() => setDialogo({ tipo: "nueva" })}>{t("Nueva carpeta")}</Button>
                </>
              )}
            </div>
          )
        ...
        principal={
          escritorio === null ? null : !disponible ? (
            <InfoBar severity="info" title={t("Esto funciona en la aplicación de escritorio")}>{t("Las carpetas de tareas son carpetas de tu computadora, así que solo están en la versión instalada de Nexo.")}</InfoBar>
          ) : (
            <>
              ...
              {!abierta && materiasSinCarpeta.length > 0 && (
                <InfoBar
                  severity="info"
                  title={materiasSinCarpeta.length === 1 ? t("Tu horario tiene 1 materia sin carpeta") : t("Tu horario tiene {n} materias sin carpeta", { n: materiasSinCarpeta.length })}
                  action={<Button onClick={() => setCrearDeHorario(true)}>{t("Crear carpetas")}</Button>}
                >
                  {t("Puedes crearlas todas de una vez, o añadirlas a mano con «Nueva carpeta».")}
                </InfoBar>
              )}
              ...
              {!abierta ? (
                    ...
                    {carpetas.map((c) => (
                      <li key={c.nombre}>
                        <div className="rounded-control reveal group flex items-center gap-2 border border-stroke bg-layer p-2 pl-3 shadow-card transition-colors duration-exit ease-fluent hover:bg-layer-alt">
                          <button type="button" onClick={() => setAbierta(c.nombre)} className="flex min-w-0 flex-1 items-center gap-3 py-1.5 text-left" aria-label={t("Abrir la carpeta {nombre}, {n} {archivos}", { nombre: c.nombre, n: c.archivos, archivos: c.archivos === 1 ? t("archivo") : t("archivos") })}>
                            ...
                            <span className="min-w-0">
                              ...
                              <span className="block truncate text-caption text-fg-secondary">
                                {c.archivos === 0 ? t("Vacía") : `${c.archivos} ${c.archivos === 1 ? t("archivo") : t("archivos")}`}
                                {c.ultima ? ` · ${fecha(c.ultima)}` : ""}
                              </span>
                            </span>
                          </button>
                          <BotonIcono glifo="editar" etiqueta={t("Renombrar {nombre}", { nombre: c.nombre })} onClick={() => setDialogo({ tipo: "renombrar-carpeta", nombre: c.nombre })} />
                          <BotonIcono glifo="eliminar" etiqueta={t("Eliminar {nombre}", { nombre: c.nombre })} peligro onClick={() => setDialogo({ tipo: "borrar-carpeta", nombre: c.nombre })} />
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                <>
                  <div>
                    <Button variant="subtle" icon={<Glifo nombre="atras" />} onClick={() => { setAbierta(null); setAviso(null); void recargar(); }}>{t("Todas las carpetas")}</Button>
                  </div>
                  <div
                    ...
                    <ul className="flex flex-col gap-1.5" aria-label={t("Archivos")}>
                      {archivos.map((a) => (
                        <li key={a.nombre} className="rounded-control flex items-center gap-2 border border-stroke bg-layer p-2 pl-3">
                          <button type="button" onClick={() => void intentar(() => abrirEnSistema(abierta, a.nombre))} className="flex min-w-0 flex-1 items-center gap-3 py-1 text-left" title={t("Abrir con su programa")} aria-label={`Abrir ${a.nombre}`}>
                          ...
                          <BotonIcono glifo="editar" etiqueta={t("Renombrar {nombre}", { nombre: a.nombre })} onClick={() => setDialogo({ tipo: "renombrar-archivo", nombre: a.nombre })} />
                          <BotonIcono glifo="eliminar" etiqueta={t("Eliminar {nombre}", { nombre: a.nombre })} peligro onClick={() => setDialogo({ tipo: "borrar-archivo", nombre: a.nombre })} />
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              {ruta && <p className="text-caption text-fg-tertiary">{t("Ubicación: {ruta}", { ruta })}</p>}
            </>
          )
        }
      />
      ...
      {dialogo?.tipo === "renombrar-carpeta" && (
        <DialogoNombre
          titulo={t("Renombrar carpeta")}
          etiqueta={t("Nuevo nombre")}
      ...
      {(dialogo?.tipo === "borrar-carpeta" || dialogo?.tipo === "borrar-archivo") && (
        <Dialog open onClose={cerrarDialogo} title={dialogo.tipo === "borrar-carpeta" ? t("¿Eliminar la carpeta?") : t("¿Eliminar el archivo?")} maxWidth={440}>
          <div className="flex flex-col gap-4">
            <p className="text-body text-fg">
              {dialogo.tipo === "borrar-carpeta"
                ? t("Se elimina «{nombre}». Solo se puede si está vacía: si tiene archivos, no se borra nada y te lo avisamos.", { nombre: dialogo.nombre })
                : t("Se elimina «{nombre}» de tu computadora. Esto no se puede deshacer.", { nombre: dialogo.nombre })}
            </p>
            <div className="flex justify-end gap-2">
              <Button onClick={cerrarDialogo}>{t("Cancelar")}</Button>
              <Button
