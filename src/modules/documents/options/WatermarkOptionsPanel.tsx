"use client";

import { SegmentedControl } from "@/components/fluent/SegmentedControl";
import { Slider } from "@/components/fluent/Slider";
import { TextInput } from "@/components/fluent/TextInput";
import { useDocumentsStore } from "@/store/documents-store";

export function WatermarkOptionsPanel() {
  const o = useDocumentsStore((s) => s.options.watermark);
  const setOption = useDocumentsStore((s) => s.setOption);
  const cambiar = (parcial: Partial<typeof o>) => setOption("watermark", { ...useDocumentsStore.getState().options.watermark, ...parcial });
  return (
    <div className="flex flex-col gap-3">
      <TextInput label="Texto" value={o.text} maxLength={40} placeholder="CONFIDENCIAL" onChange={(e) => cambiar({ text: e.target.value })} />
      <SegmentedControl
        label="Posición"
        value={o.layout}
        options={[
          { value: "diagonal", label: "En diagonal" },
          { value: "horizontal", label: "Horizontal" },
        ]}
        onChange={(layout) => cambiar({ layout })}
      />
      <SegmentedControl
        label="Tamaño"
        value={o.size}
        options={[
          { value: "small", label: "Pequeño" },
          { value: "medium", label: "Medio" },
          { value: "large", label: "Grande" },
        ]}
        onChange={(size) => cambiar({ size })}
      />
      <SegmentedControl
        label="Color"
        value={o.color}
        options={[
          { value: "gray", label: "Gris" },
          { value: "red", label: "Rojo" },
          { value: "blue", label: "Azul" },
          { value: "black", label: "Negro" },
        ]}
        onChange={(color) => cambiar({ color })}
      />
      <div>
        <div className="mb-1.5 flex justify-between text-caption text-fg-secondary">
          <span>Visibilidad</span>
          <span>{o.opacity}%</span>
        </div>
        <Slider label="Visibilidad" min={10} max={100} step={5} value={o.opacity} valueText={`${o.opacity}%`} onCommit={(opacity) => cambiar({ opacity })} />
      </div>
      <p className="text-caption text-fg-tertiary">Se pone en todas las páginas de cada archivo de la lista. Letras, números y signos habituales (sin emojis).</p>
    </div>
  );
}
