...
import { nombreDiaSemana } from "@/lib/horario/horario";
...
export function DialogoEscaneo({ abierto, onCerrar, onGuardado }: { abierto: boolean; onCerrar: () => void; onGuardado?: (materias: string[]) => void }) {
  ...
  const leer = async (archivo: File) => {
    if (!archivo.type.startsWith("image/")) {
      setFase({ tipo: "error", mensaje: t("Ese archivo no es una imagen. Usa una captura o foto del horario (PNG, JPG…).") });
      return;
    }
    setFase({ tipo: "leyendo", fraccion: 0, texto: t("Preparando el lector (la primera vez tarda unos segundos)…") });
    try {
      ...
      setFase({ tipo: "error", mensaje: e instanceof ErrorEscaneo ? e.message : t("No se pudo leer la imagen. Prueba con otra, más grande o más nítida.") });
    }
  };
  ...
  return (
    <Dialog open={abierto} onClose={onCerrar} title={t("Escanear horario")} maxWidth={640}>
      {(fase.tipo === "elegir" || fase.tipo === "error") && (
        <div className="flex flex-col gap-4">
          {fase.tipo === "error" && <InfoBar severity="error" title={t("No pude leer el horario")}>{fase.mensaje}</InfoBar>}
          <div
          ...
          >
            ...
            <p className="text-body text-fg">{t("Arrastra aquí la imagen de tu horario, o elígela")}</p>
            <Button variant="accent" onClick={() => entrada.current?.click()}>{t("Elegir imagen")}</Button>
          ...
          <p className="text-caption text-fg-secondary">
            {t("Funciona con horarios en forma de tabla: los días arriba, las horas a la izquierda y cada clase en un cuadro de color, como los de Excel. La imagen se lee en tu computadora; no se sube a ningún sitio. Cuanto más grande y nítida, mejor se lee.")}
          </p>
        </div>
      )}
      ...
      {fase.tipo === "leyendo" && (
        <div className="flex flex-col gap-3 py-6">
          ...
          <ProgressBar value={fase.fraccion * 100} label={t("Leyendo el horario")} />
        </div>
      )}
      ...
      {fase.tipo === "revisar" && (
        <div className="flex flex-col gap-4">
          <p className="text-body text-fg">
            {t("Encontré")} <strong>{fase.clases.length}</strong> {fase.clases.length === 1 ? t("clase") : t("clases")}. {t("Quita las que no sean; después de guardar puedes tocar cualquiera para corregirla.")}
          </p>
          {fase.clases.some((c) => c.dudosa) && <InfoBar severity="warning" title={t("Algunas no se leyeron bien")}>{t("Las marcadas con «Revisar» pueden tener el nombre o la clave con errores. Corrígelas después de guardar.")}</InfoBar>}
          <ul className="flex max-h-[320px] flex-col gap-1.5 overflow-y-auto pr-1" aria-label={t("Clases encontradas")}>
            {fase.clases.map((c, i) => (
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button onClick={() => setFase({ tipo: "elegir" })}>{t("Probar con otra imagen")}</Button>
            {hayHorario && <Button onClick={() => guardar("agregar")} disabled={fase.clases.length === 0}>{t("Añadir a mi horario")}</Button>}
            <Button variant="accent" onClick={() => guardar("reemplazar")} disabled={fase.clases.length === 0}>{hayHorario ? t("Reemplazar mi horario") : t("Guardar horario")}</Button>
          </div>
        </div>
      )}
    </Dialog>
