...
export function InicioMusica() {
  ...
  const empezar = async (m: Mix) => {
    ...
    setIniciando(null);
    if (!ok) avisoBreve("No se pudo empezar el mix", "Inténtalo de nuevo en un momento.");
  };

  return (
    <section aria-label="Para ti" className="flex flex-col gap-6">
      <div>
        ...
        <div className="mt-3 grid grid-cols-[repeat(auto-fill,minmax(230px,1fr))] gap-3">
          <Link href="/musica/me-gusta" className={TILE}>
            ...
            <span className="min-w-0 truncate text-body font-semibold text-fg">Canciones que te gustan</span>
          </Link>
          {tiles.map((p) => (
          ...
          <Link href="/musica/biblioteca" className={TILE}>
            ...
            <span className="min-w-0 truncate text-body font-semibold text-fg">Tu biblioteca</span>
          </Link>
        </div>
      </div>
      ...
      {mixes.length > 0 && (
        <div>
          <h2 className="mb-3 text-subtitle text-fg">Tus mixes</h2>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(170px,1fr))] gap-4">
            {mixes.map((m) => (
              <button
                ...
                aria-label={`Reproducir Mix ${m.n}, basado en ${m.artista}`}
              ...
              >
                <span className="text-body font-bold">Mix {m.n}</span>
                {m.imagen && (
                ...
                <span className="relative z-10 max-w-[58%] text-caption font-semibold leading-tight drop-shadow">{iniciando === m.n ? "Preparando…" : `Con ${m.artista} y música parecida`}</span>
              </button>
            ))}
          </div>
        </div>
