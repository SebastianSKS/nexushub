...
export function PaginaBiblioteca() {
  ...
  const deLista = useMemo<MusicItem[]>(() => playlists.map((p) => ({ kind: "playlist", id: p.id, title: p.name, subtitle: `Playlist${p.tracks !== undefined ? ` · ${p.tracks} canciones` : ""}`, cover: p.cover ?? p.image ?? "" })), [playlists]);
  ...
  return (
    <PlantillaPagina
      ...
      descripcion="Tus playlists, álbumes guardados y artistas que sigues."
      principal={
        !conectado ? (
          <InfoBar severity="info" title="Conecta tu cuenta de Spotify para ver tu biblioteca." action={<Button onClick={conectarSpotify}>Conectar</Button>}>
          ...
          <div className="flex flex-col gap-5">
            ...
            {(fallo === "permisos" || (permisos === "faltan" && pestana !== "playlists")) && (
              <AvisoPermisos texto="Para ver tus álbumes guardados y los artistas que sigues, Spotify te pide tu permiso una sola vez. Regresas justo aquí." />
            )}
            {fallo === "error" && <InfoBar severity="warning" title="No se pudo cargar. Inténtalo de nuevo en un momento." />}
            ...
            {pestana === "playlists" && (
              <div className={GRID}>
                <Link href="/musica/me-gusta" className="rounded-control reveal group block border border-stroke bg-layer p-3 text-left shadow-card transition-colors duration-exit ease-fluent hover:bg-layer-alt">
                  ...
                  <span className="mt-3 block truncate text-body font-semibold text-fg">Canciones que te gustan</span>
                  <span className="mt-0.5 block truncate text-caption text-fg-secondary">Playlist</span>
                </Link>
                {deLista.map((p) => (
            ...
            {pestana === "albumes" && (albumes === null ? !fallo && <p className="text-body text-fg-secondary" role="status">Cargando…</p> : albumes.length === 0 ? vacio("Aún no has guardado álbumes.") : (
              <div className={GRID}>
                {albumes.map((a) => (
            ...
            {pestana === "artistas" && (artistas === null ? !fallo && <p className="text-body text-fg-secondary" role="status">Cargando…</p> : artistas.length === 0 ? vacio("Aún no sigues a ningún artista. Entra a la página de uno y pulsa Seguir.") : (
              <div className={GRID}>
                {artistas.map((a) => (
                  <MusicCard key={a.id} item={a} active={false} onPlay={() => undefined} />
                ))}
