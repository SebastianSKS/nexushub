...
export function SpotifySetupDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <Dialog open={open} onClose={onClose} title="Activar Spotify Premium en NexusHub" maxWidth={620}>
      <InfoBar severity="warning" title="Necesitas Spotify Premium para esto." className="mb-4">
        Con una cuenta gratuita no se puede crear una app que funcione. Sin Premium, NexusHub usa el Modo Invitado: sugeridos, enlaces pegados y vista previa de 30 s.
      </InfoBar>
      ...
      <p className="mt-3 text-caption text-fg-tertiary">
        Nunca hace falta el Client Secret: NexusHub no lo usa ni lo guarda, porque viajaría dentro de la aplicación instalada.
      </p>
    </Dialog>
  );
}
